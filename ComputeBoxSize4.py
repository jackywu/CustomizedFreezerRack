...
class Computer:
    ...
    def compute_optimal_tubebox_size(self, tube,  tubebox, drawerbox, refrigerator):
        ...
        # 宽度之所以x2是因为此型号冰箱分为左右两个独立的空间，两空间不通
        print("冰箱内部空间尺寸: width %s x 2, depth %s, height: %s\n"
        ...
        for index in indices:
            ...
            # 如下这些数据*2的原因是4度冰箱分为左右两个独立空间
            print("离心管存储最大量为: %d, 最优解坐标为: %s"
                    % (max_tube_num*2, index))
            print("每个冻存盒可以存放离心管数量: %d" % tube_num_for_tubebox)
            print("冻存盒里的离心管排布方式为: 水平方向: %d, 深度方向: %d" % \
                    (tube_num_horizontal, tube_num_depth))
            print("冻存盒深度为: %d" % tubebox.depth)
            print("冻存盒宽度为: %d" % tubebox.width)
            print("冻存盒高度为: %d" % tubebox.height)
            print("冻存盒的管孔直径为：%d" % tubebox.get_hole_diameter(tube.diameter))
            print("冻存盒总数量为: %d" % (total_num_tubebox*2))
            ...
            print("每层水平方向抽屉架个数为: %d" % (drawer_num_horizontal*2))
            print("%s 层抽屉架总个数为: %d" % (refrigerator.division, totoal_drawerbox_num*2))
            ...
            print("每根离心管的单位耗电功率为: %.4f w" % (
                                    ...
                                    float(max_tube_num*2)))
            print("每根离心管占冰箱价格成本: %.4f 元\n" % (float(refrigerator.price*refrigerator.discount)/float(max_tube_num*2)))
        print("-"*80)
...
if __name__ == "__main__":
    ...
    tube = Tube(
            ...
            num_min_horizontal = 0,   # 水平方向最少存放离心管数量
                                      # width_min          = 34 ,  # 根据num_min_horizontal*17算出来的最小宽度
            num_max_horizontal = 0,  # 水平方向最多存放离心管数量
                                      # width_max          = 238,  # 根据num_max_horizontal*17算出来的最大宽度
            num_min_depth      = 0,   # 深度方向最少存放离心管数量
                                      # depth_min          = 34,   # 根据num_min_depth算出来的最小深度
            num_max_depth      = 0,  # 深度方向最多存放离心管数量
                                      # depth_max          = 255,  # 根据num_max_depth算出来的最大深度
            diameter           = 0    # 离心管的直径
            )
    ...
    for refrigerator in  refrigerator_list:
        computer = Computer()
        computer.compute_optimal_tubebox_size(tube, tubebox, drawerbox, refrigerator)
